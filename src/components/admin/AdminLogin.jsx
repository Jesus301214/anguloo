  ...
  const handleEmail = async (e) => {
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text:
          err.message === 'Invalid login credentials' ? 'Credenciales incorrectas.' : err.message,
      })
    } finally {
      setIsLoading(false)
    }
    ...
    <div className="min-h-screen bg-[#0F172A] flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-[#1E293B] border border-slate-800 rounded-3xl p-10 shadow-2xl">
        ...
        <div className="space-y-6">
          {message && (
            <div
              className={`rounded-2xl p-4 text-sm font-bold text-center ${message.type === 'error' ? 'bg-rose-500/10 border border-rose-500/20 text-rose-400' : 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400'}`}
            >
              {message.text}
            </div>
          )}
