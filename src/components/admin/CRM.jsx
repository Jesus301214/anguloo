    ...
    <div className="space-y-6 animate-in fade-in duration-700 bg-slate-950 p-6 rounded-3xl border border-slate-800/50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-400 mt-1 font-medium">Gestiona prospectos y agenda reuniones estratégicas.</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={() => alert('Abriendo Calendario de Reuniones...')} className="flex items-center gap-2 px-4 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-bold hover:bg-emerald-700 shadow-lg shadow-emerald-600/20 transition-all"><Calendar size={18} /> Agendar Reunión</button>
        </div>
      </div>
      ...
      <div className="relative w-full">
        ...
        <input 
          type="text" 
          placeholder="Buscar prospectos por nombre, empresa o email..." 
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="w-full pl-12 pr-4 py-4 bg-slate-900/50 border border-slate-800 focus:border-blue-500/50 rounded-2xl text-slate-200 outline-none transition-all placeholder:text-slate-600"
        />
      </div>
      ...
      <div className="bg-slate-900/40 rounded-2xl border border-slate-800 overflow-hidden">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-24 text-slate-500"><Loader2 className="animate-spin mb-4" size={40} /><p className="font-bold tracking-widest uppercase text-xs">Sincronizando datos...</p></div>
        ) : (
                  ...
                  <tr key={lead.id} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button onClick={() => alert('Agendando reunión con ' + lead.nombre)} className="p-2.5 bg-purple-500/10 text-purple-500 rounded-xl hover:bg-purple-500 hover:text-white transition-all border border-purple-500/20" title="Agendar"><Calendar size={18} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-3xl p-8 shadow-2xl relative animate-in zoom-in duration-300">
            ...
            <h2 className="text-2xl font-black text-white mb-6 font-outfit">Crear Nuevo Prospecto</h2>
            <form onSubmit={handleCreateLead} className="space-y-4">
              <div><label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">Nombre Completo</label>
