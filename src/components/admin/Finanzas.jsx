...
const Finanzas = () => {
  ...
  const fetchTxns = async () => {
    ...
    try {
      ...
      logger.error('Finanzas.fetchTxns', err)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSave = async (e) => {
    ...
    try {
      const { error } = await supabase
        .from('transactions')
        .insert([{ ...form, monto: parseFloat(form.monto) || 0 }])
      if (error) throw error
      ...
      logger.error('Finanzas.handleSave', err)
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async (id) => {
    try {
      ...
      logger.error('Finanzas.handleDelete', err)
    }
  }
    ...
    <div className="space-y-8 bg-[#0F172A] p-8 rounded-3xl border border-slate-800/40 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-white font-outfit flex items-center gap-3">
            <Wallet className="text-emerald-500" /> Finanzas
          </h1>
          <p className="text-slate-400 mt-1 font-medium">Control de ingresos y gastos.</p>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="flex items-center gap-2 px-4 py-2.5 bg-emerald-500 text-white rounded-xl text-sm font-bold hover:bg-emerald-600 shadow-lg transition-all"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
          ...
          <p
            className={`text-3xl font-black mt-2 ${balance >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
          >
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
        ...
        <div className="flex items-end gap-3 h-32">
          {monthData.map((m, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-1">
              ...
              >
                <div
                  className="w-3 bg-emerald-500/60 rounded-t transition-all"
                  style={{ height: `${(m.ingreso / maxVal) * 100}%`, minHeight: '4px' }}
                  title={`Ingreso: $${m.ingreso}`}
                />
                <div
                  className="w-3 bg-rose-500/60 rounded-t transition-all"
                  style={{ height: `${(m.gasto / maxVal) * 100}%`, minHeight: '4px' }}
                  title={`Gasto: $${m.gasto}`}
                />
              </div>
        ...
        <div className="flex items-center justify-center gap-6 mt-4 text-[10px] text-slate-500 font-bold">
          <span className="flex items-center gap-1">
            <div className="w-2 h-2 rounded bg-emerald-500" /> Ingresos
          </span>
          <span className="flex items-center gap-1">
            <div className="w-2 h-2 rounded bg-rose-500" /> Gastos
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-4 bg-slate-800/40 border border-slate-700/50 rounded-2xl text-slate-200 outline-none focus:border-emerald-500/30 transition-all placeholder:text-slate-500/50 font-medium"
          />
        </div>
        <div className="flex gap-2">
          {['all', 'ingreso', 'gasto'].map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${filter === f ? 'bg-blue-600 text-white' : 'bg-slate-800/40 text-slate-500 hover:text-white border border-slate-700/50'}`}
            >
              ...
              {filtered.map((t) => (
                <tr key={t.id} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-6 py-5">
                    <div
                      className={`flex items-center gap-2 ${t.tipo === 'ingreso' ? 'text-emerald-400' : 'text-rose-400'} text-xs font-black uppercase`}
                    >
                      {t.tipo === 'ingreso' ? (
                  ...
                  <td className="px-6 py-5">
                    <span className="text-xs bg-slate-800 text-slate-400 px-3 py-1 rounded-lg font-bold">
                      {t.categoria || '—'}
                    </span>
                  </td>
                  <td className="px-6 py-5 text-sm text-slate-400">{t.fecha}</td>
                  <td
                    className={`px-6 py-5 text-right text-sm font-black ${t.tipo === 'ingreso' ? 'text-emerald-400' : 'text-rose-400'}`}
                  >
                    {t.tipo === 'ingreso' ? '+' : '-'}${(t.monto || 0).toLocaleString()}
                  </td>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div className="flex gap-2">
                {['ingreso', 'gasto'].map((t) => (
                  <button
                    ...
                    className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${form.tipo === t ? (t === 'ingreso' ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white') : 'bg-slate-800 text-slate-500 border border-slate-700'}`}
                  >
                    {t}
                  </button>
                ))}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3.5 text-white outline-none focus:border-emerald-500/50 text-2xl font-black"
                  placeholder="0.00"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3.5 text-white outline-none focus:border-emerald-500/50"
              ...
              <button
                disabled={saving}
                type="submit"
                className={`w-full ${form.tipo === 'ingreso' ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-rose-500 hover:bg-rose-600'} text-white font-black py-4 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 mt-2`}
              >
