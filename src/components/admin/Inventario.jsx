    ...
    <div className="space-y-8 bg-[#0F172A] p-8 rounded-3xl border border-slate-800/40 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-white font-outfit flex items-center gap-3">
            <Package className="text-amber-500" /> Inventario
          </h1>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-amber-500 text-white rounded-xl text-sm font-bold hover:bg-amber-600 shadow-lg transition-all"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
          ...
          <p className="text-3xl font-black text-white mt-2">{items.length}</p>
        </div>
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
          ...
          <p className="text-3xl font-black text-rose-400 mt-2">{lowStock}</p>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-12 pr-4 py-4 bg-slate-800/40 border border-slate-700/50 rounded-2xl text-slate-200 outline-none focus:border-amber-500/30 transition-all placeholder:text-slate-500/50 font-medium"
        />
      </div>
              ...
              {filtered.map((item) => (
                <tr key={item.id} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-6 py-5">
                    <p className="text-sm font-black text-white">{item.nombre}</p>
                  </td>
                  <td className="px-6 py-5">
                  ...
                  <td className="px-6 py-5">
                    <span
                      className={`text-sm font-black ${(item.stock || 0) <= 5 ? 'text-rose-400' : 'text-white'}`}
                    >
                      {item.stock || 0}{' '}
                      {(item.stock || 0) <= 5 && (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-3xl p-8 shadow-2xl relative">
            ...
            <h2 className="text-xl font-black text-white mb-6 font-outfit">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3.5 text-white outline-none focus:border-amber-500/50"
                />
              </div>
              <div>
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-1">
              ...
              <button
                disabled={saving}
                type="submit"
                className="w-full bg-amber-500 hover:bg-amber-600 text-white font-black py-4 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 mt-2"
              >
