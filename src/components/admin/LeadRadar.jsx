  ...
  return (
    <div className="space-y-8 bg-[#F8FAFC] p-8 rounded-3xl min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-slate-900 font-outfit flex items-center gap-3"><Radar className="text-blue-600 animate-pulse" /> LeadRadar B2B</h1>
          <p className="text-slate-500 mt-1 font-bold uppercase tracking-widest text-[10px]">Doble motor de prospección — 100% gratuito.</p>
        </div>
      ...
      <div className="flex gap-2 bg-slate-100 p-1.5 rounded-2xl border border-slate-200 w-fit shadow-sm">
        <button onClick={() => setEngine('osm')} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${engine==='osm'?'bg-blue-600 text-white shadow-lg shadow-blue-600/20':'text-slate-500 hover:text-slate-900'}`}><MapPin size={14} /> Geolocalización</button>
        <button onClick={() => setEngine('web')} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${engine==='web'?'bg-purple-600 text-white shadow-lg shadow-purple-600/20':'text-slate-500 hover:text-slate-900'}`}><Globe size={14} /> Web Search</button>
      </div>
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
        <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder={engine==='osm'?'Ej: Salones de belleza en Caracas':'Ej: Spas premium Caracas contacto'} className="w-full bg-slate-50 border border-slate-100 rounded-2xl pl-12 pr-4 py-4 text-slate-900 outline-none focus:bg-white focus:border-blue-500/30 transition-all placeholder:text-slate-400 font-bold text-sm" />
          </div>
          <button type="submit" disabled={isLoading||!query.trim()} className={`${engine==='osm'?'bg-blue-600 hover:bg-blue-700 shadow-blue-600/20':'bg-purple-600 hover:bg-purple-700 shadow-purple-600/20'} text-white font-black px-8 py-4 rounded-2xl shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        list.map(biz => {
          ...
          return (
            <div key={biz.id} className="group bg-white border border-slate-200 hover:border-blue-500/30 rounded-[2rem] p-7 transition-all flex flex-col justify-between shadow-sm hover:shadow-xl hover:-translate-y-1">
              <div>
                ...
                <h3 className="text-lg font-black text-slate-900 leading-tight mb-2 group-hover:text-blue-600 transition-colors line-clamp-2 uppercase tracking-tight">{biz.name}</h3>
                {biz.full_address && <p className="text-slate-500 text-[11px] font-bold mb-3 line-clamp-1 flex items-center gap-1.5"><MapPin size={12} className="text-slate-300" /> {biz.full_address}</p>}
                {biz.description && <p className="text-slate-500 text-xs mb-3 line-clamp-3 leading-relaxed font-medium">{biz.description}</p>}
                <div className="space-y-2 mb-6">
                  {biz.phone && <div className="flex items-center gap-2 text-slate-600 text-xs font-bold bg-slate-50 p-2 rounded-lg border border-slate-100"><Phone size={14} className="text-slate-400" />{biz.phone}</div>}
                  {(biz.website||biz.url) && <div className="flex items-center gap-2 text-xs font-bold bg-slate-50 p-2 rounded-lg border border-slate-100"><Globe size={14} className="text-slate-400" /><a href={(biz.website||biz.url).startsWith('http')?(biz.website||biz.url):`https://${biz.website||biz.url}`} target="_blank" rel="noreferrer" className="truncate text-blue-600 hover:underline">{(biz.website||biz.url).replace(/^https?:\/\//,'')}</a></div>}
                </div>
              </div>
              <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                {biz.lat && <a href={`https://www.google.com/maps?q=${biz.lat},${biz.lon}`} target="_blank" rel="noreferrer" className="p-2.5 bg-slate-50 text-slate-400 rounded-xl hover:bg-blue-600 hover:text-white transition-all border border-slate-100"><ExternalLink size={18}/></a>}
                {biz.url && !biz.lat && <a href={biz.url} target="_blank" rel="noreferrer" className="p-2.5 bg-slate-50 text-slate-400 rounded-xl hover:bg-purple-600 hover:text-white transition-all border border-slate-100"><ExternalLink size={18}/></a>}
